% For exp 2.2 Lagrangian freq estimation: pose estimation at all frames + Tracker +  FFT over joints (x,y) ( With SMOOTH).

path = '/tudelft.net/staff-bulk/ewi/insy/VisionLab/students/jianzheng/smooth_trajectory/';
video_code_list = dir(path);
results_save_path = '/tudelft.net/staff-bulk/ewi/insy/VisionLab/students/jianzheng/exp_2_2/';
...
for i = 3:length(video_code_list)
    ...
    for j = 3 : length(task_list)
        ...
        smoothedFile_path = strcat(task_folder_path,'/',task_folder_name);
        fid = fopen(smoothedFile_path);
        smooth_traj = textscan(fid, '%f %f');
        fclose(fid);
        if isempty(smooth_traj)
        ...
        psd = [];
		for k = 1:2
		    Signal = smooth_traj{k};
		    [~, freq_,~] = AccToFreq(Signal,sampleFreq,windowSize);
		    [~, psd_,isPeak_overall_] = AccToFreqPSD(Signal,sampleFreq,windowSize);
		    if isempty(freq)
		        freq = freq_.^2;
		    else
		        freq = freq+freq_.^2;
        ...
        count = length(freq);
        task_name = strsplit(task_folder_name,'.');
        result_path = strcat(code_result_save_path,'/',task_name{1},'/');
