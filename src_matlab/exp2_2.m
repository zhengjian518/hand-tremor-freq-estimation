...
results_save_path = '/tudelft.net/staff-bulk/ewi/insy/VisionLab/students/jianzheng/exp_2_2_update/';
...
for i = 3:length(video_code_list)
    ...
    for j = 3 : length(task_list)
        ...
        smoothed_traj_path = strcat(smoothed_path, patient_folder_name,'/', task_folder_name, '.txt')
        fid = fopen(smoothed_traj_path);
        smooth_traj_cell = textscan(fid, '%f %f')
        fclose(fid);
        ...
        new_pos_cell = cell(1,2);
        new_pos_cell{1} = signal_cell{1} - smooth_traj_cell{1};
        new_pos_cell{2} = signal_cell{2};

        freq = []; % sqrt(/3)
        isPeak = [];
        psd = [];
        for k = 1:2
            accSignal = new_pos_cell{k};
