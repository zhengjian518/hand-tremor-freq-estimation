...
class TFD_PHASE():
	...
	@staticmethod
	def tfd_batch_phase(video_path_list,window_size_list,joint_list):
		...
		for i in range(len(video_path_list)):
			...
			TFD_PHASE.tremor_freq_detec_phase(video_path,window_size,noverlap,fft_video_path,final_freq_csv_path,joint_list)
	...
	def tremor_freq_detec_phase(video_path,window_size,noverlap, 
			...
			for i in range(0,int(video_fft.FRAME_COUNT/stride)-1):
				...
				for joint_i in JOINT_LIST:# range(JOINTS_NUM):
					...
					for j in range(0,len(phase_image_top)): # 13
						...
						if j < 5:
							conf_maps_cropped = conf_maps[pred_y-box_size:pred_y+box_size,
							...
							conf_maps_cropped = conf_maps[pred_y-box_size/2:pred_y+box_size/2,
														pred_x-box_size/2:pred_x+box_size/2,
														joint_i]
							conf_maps_cropped = conf_maps_cropped / np.sum(conf_maps_cropped) 
							joint_conf_maps_inner.append(conf_maps_cropped)
							...
							assert j <= 12
							conf_maps_cropped = conf_maps[(pred_y-box_size/4-1):(pred_y+box_size/4),
														(pred_x-box_size/4-1):(pred_x+box_size/4),
														joint_i]
							conf_maps_cropped = conf_maps_cropped / np.sum(conf_maps_cropped) 
							joint_conf_maps_inner.append(conf_maps_cropped)
		...
		while( pe_process.is_alive() or fft_process.is_alive() or \
			...
			if( (not joint_conf_map_queue.empty()) and \
				...
				for joint_i in range(len(freq_results)): # joint level
					# writer file XXX_tfd_freq.csv
					csvwriter_head = []
					for phase_index in range(len(freq_results[joint_i])/2):
						csvwriter_head.extend(['c{}_{}_P{}'.format(fft_count,\
															joints_string[joint_i],phase_index),'is_peak'])
